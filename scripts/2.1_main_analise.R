...
# Pipeline completo (SEM LEITURA de bruto):
# - Temperatura (T): visão geral, por dia, painéis por período,
#   por linha, por coluna (5x2 + individuais), mapas temporais
# - Umidade (UR): MESMOS ENTREGÁVEIS (espelhados)
# - Falhas: métricas (csv + rds), paretos (simples/linha/coluna),
#   falhas por sensor (gráficos) para múltiplos limites
#
# Requer:
...
# limites em minutos
limites_metricas <- c(15, 30, 60)     # usados em métricas + paretos
bins_map_min <- c(5, 10, 60)          # usados nos mapas temporais
limites_plot_falhas <- c(5, 10, 60)   # usados nos gráficos por sensor
...
periodos <- tibble(
  ...
  inicio = ymd_hm(
    c(
      "2025-07-21 10:10", "2025-07-23 07:00", "2025-07-26 07:00",
  ...
  fim = ymd_hm(
    c(
      "2025-07-23 07:00", "2025-07-26 07:00", "2025-07-28 07:00",
...
# faixa ideal apenas para TEMPERATURA (como você já tinha)
faixas_ideais_t <- tibble(
  Periodo = c("Alojamento", "Abertura1", "Abertura2", "Abertura3", "Abertura4", "Abertura5"),
  Tmin = c(32, 32, 32, 29, 26, 23),
  Tmax = c(35, 35, 35, 32, 29, 26)
)

# (opcional) faixa ideal para UMIDADE por período
# Quando você souber os valores, preencha e o retângulo volta automaticamente.
faixas_ideais_ur <- tibble(
  Periodo = c("Alojamento", "Abertura1", "Abertura2", "Abertura3", "Abertura4", "Abertura5"),
  URmin = rep(NA_real_, 6),
  URmax = rep(NA_real_, 6)
)
...
datas_aberturas <- periodos |>
  select(evento = Periodo, data_hora = inicio)

# ============================================================
# 7) FUNÇÕES GENÉRICAS PARA SÉRIE (T ou UR)
# ============================================================
get_spec <- function(var) {
  # Define labels/pastas por variável
  if (var == "T") {
    list(
      var = "T",
      nome = "temperatura",
...
plot_geral <- function(df, spec, lote_id, datas_aberturas) {
  y_vals <- df[[spec$var]]
  ylim_max <- max(y_vals, na.rm = TRUE) + 1
  
  ggplot(df, aes(x = data_hora, y = .data[[spec$var]], color = factor(Sensor))) +
    ...
    coord_cartesian(
      ylim = c(min(y_vals, na.rm = TRUE), ylim_max + 1.5),
      clip = "off"
    ) +
    labs(
...
plot_por_dia <- function(df, spec, lote_id) {
  ggplot(df, aes(x = data_hora, y = .data[[spec$var]], color = factor(Sensor), group = Sensor)) +
...
plot_painel_periodo <- function(df, spec, nome_periodo, quebra_horas = "4 hours") {
  ...
  n_dias <- n_distinct(dados_p$Data)
  ...
  # faixa ideal: só desenha se existir e não for NA
  if (spec$var == "T") {
    faixa <- faixas_ideais_t |> filter(Periodo == nome_periodo)
    usar_faixa <- nrow(faixa) == 1 && is.finite(faixa$Tmin) && is.finite(faixa$Tmax)
    
    faixa_dias <- dados_p |>
      group_by(Data) |>
      summarise(xmin = min(hora_dia), xmax = max(hora_dia), .groups = "drop") |>
      mutate(ymin = faixa$Tmin, ymax = faixa$Tmax)
  } else {
    faixa <- faixas_ideais_ur |> filter(Periodo == nome_periodo)
    usar_faixa <- nrow(faixa) == 1 && is.finite(faixa$URmin) && is.finite(faixa$URmax)
    
    faixa_dias <- dados_p |>
      group_by(Data) |>
      summarise(xmin = min(hora_dia), xmax = max(hora_dia), .groups = "drop") |>
      mutate(ymin = faixa$URmin, ymax = faixa$URmax)
  }
  
  g <- ggplot(dados_p, aes(x = hora_dia, y = .data[[spec$var]], color = Sensor_f, group = Sensor_f))
  ...
  subtitle_txt <- if (usar_faixa) {
    if (spec$var == "T") {
      glue(
        "Sensores: {sens_range$sensor_min}–{sens_range$sensor_max} | ",
        "Período: {p$inicio} até {p$fim} | Faixa ideal: {faixa$Tmin}–{faixa$Tmax}"
      )
    } else {
      glue(
        "Sensores: {sens_range$sensor_min}–{sens_range$sensor_max} | ",
        "Período: {p$inicio} até {p$fim} | Faixa ideal: {faixa$URmin}–{faixa$URmax}"
      )
    }
  } else {
    glue(
...
plot_por_linha <- function(df, spec, lote_id, datas_aberturas = NULL) {
  y_max <- max(df[[spec$var]], na.rm = TRUE)
  
  g <- ggplot(df, aes(x = data_hora, y = .data[[spec$var]], color = factor(Sensor), group = Sensor)) +
    geom_line(linewidth = 0.55, alpha = 0.85) +
    facet_wrap(~Linha, ncol = 1) +
    scale_cor_sensor_padrao("Sensor") +
    labs(
      title = glue("Sensores por linha — {spec$titulo} — {lote_id}"),
      x = "Tempo",
      y = spec$ylab
    ) +
...
plot_por_coluna_5x2 <- function(df, spec, lote_id, datas_aberturas = NULL) {
  y_max <- max(df[[spec$var]], na.rm = TRUE)
...
salvar_por_coluna <- function(df, spec, lote_id, datas_aberturas, dir_out) {
  ...
  for (cc in colunas) {
    ...
    g <- ggplot(d, aes(x = data_hora, y = .data[[spec$var]], color = factor(Sensor), group = Sensor)) +
      ...
      scale_cor_sensor_padrao("Sensor") +
      labs(
        title = glue("{cc} — {spec$titulo} — {lote_id}"),
        x = "Tempo",
        y = spec$ylab
      ) +
      theme_minimal(base_size = 12) +
...
# ============================================================
# 8) EXECUTA ENTREGÁVEIS DE SÉRIE (T e/ou UR) — TUDO ESPELHADO
# ============================================================
rodar_entregaveis_serie <- function(df, var) {
  spec <- get_spec(var)
  
  # Pastas por variável (T e UR)
  ...
  # 7) visão geral
  g_geral <- plot_geral(df, spec, lote_id, datas_aberturas)
  print(g_geral)
  ...
  # 12) por linha
  g_linha <- plot_por_linha(df, spec, lote_id, datas_aberturas)
  print(g_linha)
  save_plot(g_linha, file.path(dir_main, glue("02_{spec$nome}_por_linha.png")), w = 16, h = 7)
  
  # 13) por coluna 5x2
  g_col_5x2 <- plot_por_coluna_5x2(df, spec, lote_id, datas_aberturas)
  print(g_col_5x2)
  save_plot(g_col_5x2, file.path(dir_main, glue("03_{spec$nome}_por_coluna_5x2.png")), w = 18, h = 12)
  
  # 13b) colunas individuais
  salvar_por_coluna(df, spec, lote_id, datas_aberturas, dir_colunas_var)
  ...
  for (bm in bins_map_min) {
    ...
    g_mapa <- plot_mapa_temporal_registros(df, lote_id, bin = bin_str)
    print(g_mapa)
    save_plot(g_mapa, file.path(dir_mapas_var, glue("04_mapa_registros_{bm}min.png")), w = 18, h = 9)
    
    g_mapa_pres <- plot_mapa_temporal_presenca(df, lote_id, bin = bin_str)
    print(g_mapa_pres)
...
# -------------------------------
# 9) EXECUTA T e UR (sem mexer falhas)
# -------------------------------
...
# (mantém ENTREGÁVEIS exatamente como seu script original)
# ============================================================
dir_tabs <- dir_result_lote(pasta_base, lote_id, "tabelas")
...
# (mantém ENTREGÁVEIS exatamente como seu script original)
# ============================================================
dir_pareto <- dir_result_lote(pasta_base, lote_id, "falhas", "pareto")
...
for (lf in limites_metricas) {
  gerar_paretos_limite(lf)
}

# ============================================================
# 15) FALHAS DE REGISTRO POR SENSOR (linha + pontos vermelhos)
# (mantém ENTREGÁVEL exatamente como seu script original)
# ============================================================
...
for (lim in limites_plot_falhas) {
  for (sid in sensores_todos) {
    g <- plot_falhas_sensor(
      ...
      limite_min = lim,
      lote_id = lote_id,
      datas_aberturas = datas_aberturas
    )
    if (is.null(g)) next
